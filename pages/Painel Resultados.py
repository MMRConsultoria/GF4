...
if agrupamento == "Dia" and modo_visao == "Por Grupo":
    ...
    for data_selecionada in pd.date_range(start=data_inicio, end=data_fim):
        ...
        if "Grupo_x" in df_dia_merged.columns:
            df_dia_merged["Grupo"] = df_dia_merged["Grupo_x"]
        if "Grupo_y" in df_dia_merged.columns:
            df_dia_merged["Grupo"] = df_dia_merged["Grupo"].combine_first(df_dia_merged["Grupo_y"])
        df_dia_merged.drop(columns=[col for col in df_dia_merged.columns if col.startswith("Grupo_")], inplace=True)

        # Preenche valores ausentes
        for col in ["Fat.Total", "Fat.Real", "Serv/Tx", "Ticket"]:
            if col in df_dia_merged.columns:
                df_dia_merged[col] = df_dia_merged[col].fillna(0)

        # Colunas auxiliares
        df_dia_merged["Ano"] = data_selecionada.year
        df_dia_merged["Mês Num"] = data_selecionada.month
        df_dia_merged["Mês Nome"] = data_selecionada.strftime('%B')
        df_dia_merged["Mês"] = data_selecionada.strftime('%m/%Y')
        df_dia_merged["Dia"] = data_selecionada.strftime('%d/%m/%Y')
        df_dia_merged["Agrupador"] = data_selecionada.strftime('%d/%m/%Y')
        df_dia_merged["Ordem"] = data_selecionada

        lista_dfs.append(df_dia_merged)

    # ✅ Junta todos os dias no final
