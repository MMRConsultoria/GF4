...
if modo_visao == "Por Loja":
    ...
    if 'index' in tabela_exportar.columns:
        tabela_exportar = tabela_exportar.rename(columns={'index': 'Loja'})
    elif tabela_exportar.columns[0] != 'Loja':
    ...
    tabela_exportar = tabela_final.reset_index()
    if 'index' in tabela_exportar.columns:
        tabela_exportar = tabela_exportar.rename(columns={'index': 'Grupo'})
    elif tabela_exportar.columns[0] != 'Grupo':
...
with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
    ...
    for grupo_atual in tabela_exportar_sem_tipo["Grupo"].unique():
        ...
        subtotal_grupo = grupo_linhas.iloc[:, 2:].sum().sum()
