...
if modo_visao == "Por Loja":
    ...
    tabela_exportar = tabela_exportar.merge(
        relacao_loja, on="Loja", how="left"
    )

    cols = ["Grupo", "Loja", "Tipo"] + [c for c in tabela_exportar.columns if c not in ["Grupo", "Loja", "Tipo"]]
...
# 🔥 Cópia sem a coluna Tipo (para download principal)
tabela_exportar_sem_tipo = tabela_exportar.drop(columns="Tipo", errors="ignore")


# 🔥 Acumulado no Mês, SOMENTE quando agrupamento == "Dia"
if agrupamento == "Dia":
    try:
        # 🎯 Filtra dados acumulados até a data selecionada
        data_maxima = pd.to_datetime(data_fim)
        df_acumulado = df_anos[
            (df_anos["Data"].dt.year == data_maxima.year) &
            (df_anos["Data"].dt.month == data_maxima.month) &
            (df_anos["Data"].dt.day <= data_maxima.day)
        ].copy()
        ...
        # 🎯 Acumulado por Loja
        if modo_visao == "Por Loja":
            df_loja = df_acumulado.groupby("Loja")["Fat.Real"].sum().reset_index()
            df_loja.rename(columns={"Fat.Real": "Acumulado no Mês"}, inplace=True)
            tabela_exportar_sem_tipo = tabela_exportar_sem_tipo.merge(df_loja, on="Loja", how="left")

        # 🎯 Acumulado por Grupo
        elif modo_visao == "Por Grupo":
            df_grupo = df_acumulado.groupby("Grupo")["Fat.Real"].sum().reset_index()
            df_grupo.rename(columns={"Fat.Real": "Acumulado no Mês"}, inplace=True)
            tabela_exportar_sem_tipo = tabela_exportar_sem_tipo.merge(df_grupo, on="Grupo", how="left")

        # 🎯 Acumulado por Tipo
        df_tipo = df_acumulado.groupby("Tipo")["Fat.Real"].sum().reset_index()
        df_tipo.rename(columns={"Fat.Real": "Acumulado no Mês Tipo"}, inplace=True)
        tabela_exportar_sem_tipo = tabela_exportar_sem_tipo.merge(df_tipo, on="Tipo", how="left")

        # 🎯 Organiza ordem das colunas
        cols_atuais = [c for c in tabela_exportar_sem_tipo.columns if c not in ["Acumulado no Mês", "Acumulado no Mês Tipo"]]
        tabela_exportar_sem_tipo = tabela_exportar_sem_tipo[cols_atuais + ["Acumulado no Mês", "Acumulado no Mês Tipo"]]

    except Exception as e:
        st.warning(f"⚠️ Erro no cálculo do acumulado: {e}")
...
with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
    tabela_exportar_sem_tipo.to_excel(writer, sheet_name="Faturamento", index=False)

    workbook = writer.book
    worksheet = writer.sheets["Faturamento"]

    cores = itertools.cycle(["#D9EAD3", "#CFE2F3"])

    header_format = workbook.add_format({'bold': True, 'bg_color': '#4F81BD', 'font_color': 'white', 'align': 'center', 'border': 1})
    subtotal_format = workbook.add_format({'bold': True, 'bg_color': '#FFE599', 'border': 1, 'num_format': 'R$ #,##0.00'})
    totalgeral_format = workbook.add_format({'bold': True, 'bg_color': '#A9D08E', 'border': 1, 'num_format': 'R$ #,##0.00'})

    for col_num, value in enumerate(tabela_exportar_sem_tipo.columns):
        worksheet.write(0, col_num, value, header_format)

    # ✅ Subtotal por Grupo
    grupos = tabela_exportar_sem_tipo["Grupo"].unique()
    linha = 1

    for grupo in grupos:
        dados = tabela_exportar_sem_tipo[tabela_exportar_sem_tipo["Grupo"] == grupo]
        cor = next(cores)

        formato = workbook.add_format({'bg_color': cor, 'border': 1, 'num_format': 'R$ #,##0.00'})

        for _, row in dados.iterrows():
            for col_num, val in enumerate(row):
                if isinstance(val, (float, int)) and not pd.isna(val):
                    worksheet.write_number(linha, col_num, val, formato)
                else:
                    worksheet.write(linha, col_num, str(val), formato)
            linha += 1

        subtotal = dados.iloc[:, 2:].sum()
        linha_subtotal = [f"Subtotal {grupo}", f"Lojas: {dados['Loja'].nunique()}"] + list(subtotal)

        for col_num, val in enumerate(linha_subtotal):
            if isinstance(val, (float, int)):
                worksheet.write_number(linha, col_num, val, subtotal_format)
            else:
                worksheet.write(linha, col_num, val, subtotal_format)

        linha += 1

    # ✅ Total Geral
    total = tabela_exportar_sem_tipo.iloc[:, 2:].sum()
    linha_total = ["Total Geral", ""] + list(total)

    for col_num, val in enumerate(linha_total):
        if isinstance(val, (float, int)):
            worksheet.write_number(linha, col_num, val, totalgeral_format)
        else:
            worksheet.write(linha, col_num, val, totalgeral_format)
